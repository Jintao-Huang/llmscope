...
2. `INFONCE_IN_BATCH_NEG`: Use `negative_messages` within the sample (hard negatives) or use other samples in the batch as in-batch negatives. The default is `True`, which means using in-batch negatives.
...
>
> `negative_messages` can be omitted. In this case, keep `INFONCE_IN_BATCH_NEG=True` to use in-batch negatives (other samples in the batch) as negatives.
